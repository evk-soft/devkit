...
Status: approved target architecture; the R4 formatter-provider correction was approved by the
repository owner on 2026-08-02, and implementation is delivered through separately approved stages
Date: 2026-08-01
...
| Formatter provider | Preserve the caller-explicit direct executable form and add a caller-explicit root-declared Node package entry; resolve either to one absolute file before launch | AI Tooling passes exact argv with `shell: false` and a disposable-repository working directory; Windows command shims cannot force a shell fallback and no provider is fetched or inferred |
| Pack executable trust | Make instruction-only pack resolution the default and require separate source trust and capability consent for activatable pack assets | Pack scripts, hooks, MCP servers, executables, connectors, and browser assets fail closed; caller-explicit formatter providers use the separate formatter-provider trust contract |
...
`doctor --formatter-check` has two explicit, formatter-independent provider forms:
...
The direct form preserves the caller-supplied executable token and exact argument array. Before
changing working directory, AI Tooling resolves a path-like token against the original repository
root or resolves a bare token once through the caller's `PATH` by direct lookup, not shell command
construction. It realpaths the result, requires an existing regular file that the operating system
can launch directly, records its
identity, and revalidates that identity immediately before spawn. On Windows, `.cmd`, `.bat`, `.ps1`,
and `.psm1` command wrappers and Windows drive-relative, UNC, device, or URL paths are invalid rather
than reasons to enable a shell or network lookup. A direct provider inside the repository is a repository-content input and
passes the containment gateway; one outside the repository is accepted only under caller-explicit
operational trust. The frozen absolute result, not the caller's `PATH`, is used for launch. Bare-token
mode intentionally treats the caller's `PATH` as provider-selection input before freezing; no other
environment discovery may select or alter the provider. This preserves direct native executable
lookup without allowing platform command shims to change the launch mechanism.

The Node-entry form is for an explicitly selected installed JavaScript command. It parses the package
name from the exact, non-normalized bare package specifier, requires that name as an own entry of the
repository-root manifest's `dependencies`, `devDependencies`, or `optionalDependencies`, and requires
the corresponding original-root `node_modules/<package-name>` installation entry. Built-ins, package
imports, relative, absolute, drive-relative, UNC, `file:`, `node:`, URL, percent-encoded, backslash,
empty-component, and dot-segment specifiers are invalid. The root manifest is read through the
repository-content containment gateway.

AI Tooling requires the original-root `node_modules` anchor itself to be an existing non-link,
non-reparse directory whose real path is contained by the real repository root. It records the
installation entry's real package root, which must remain inside that anchored real `node_modules`
tree, and resolves through
`createRequire(<absolute-original-root>/package.json).resolve(<module-specifier>)`. It realpaths the
result and accepts it only when it is an absolute existing regular file contained within the recorded
package root. A missing local installation, parent or global fallback, cross-package export, and
non-file result fail closed. This rule supports a repository-local pnpm junction while rejecting a
dependency link that escapes the installed tree. AI Tooling records the contained root manifest's
byte digest and dependency declaration. Immediately before spawn, it re-reads that manifest through
the gateway and revalidates its digest and declaration, plus the `node_modules` anchor, installation
entry, real package root, resolved-entry identity, and every containment relationship.

AI Tooling resolves either provider before changing working directory. Direct mode launches the
frozen absolute executable; Node-entry mode launches `process.execPath` followed by the frozen
absolute entry. Both receive the exact caller argument array, `shell: false`, and the disposable
repository as `cwd`. AI Tooling never constructs a command string, inserts an intermediary shell, or
falls back to one. The disposable copy contains no link to the original `node_modules`; a Node entry
and its dependencies continue to load from the original installed graph under their separate trust
contract.
...
`NO_COLOR=1`. On Windows, `USERPROFILE`, `APPDATA`, and `LOCALAPPDATA` are disposable. A trusted
operating-system API supplies one canonical Windows directory; `SystemRoot` and `WINDIR` receive that
same value, `PATH` contains only that directory, its `System32`, and `System32\\Wbem`, and `PATHEXT` is
`.COM;.EXE`. Caller environment cannot supply or override those values. On POSIX, `PATH` is
exactly `/usr/bin:/bin`, and `LANG` and `LC_ALL` are `C`. No inherited variable is passed, including
`NODE_OPTIONS`, `NODE_PATH`, `BIOME_BINARY`, package-manager variables, proxy settings, credentials,
or tokens. Stage 1 has no formatter environment passthrough.

Neither form lets AI Tooling infer a provider, acquire a package, or perform a package/network fetch
or resolver request. A caller-selected filesystem location, including mounted storage, remains under
caller trust. Pack content and AI configuration cannot select a command. The explicitly selected provider and its
transitive behavior are caller-trusted operational code. Stage 1 does not claim that this code is
shell-free, network-free, or OS-sandboxed. AI Tooling opens no real-checkout path for write and never
supplies the real repository as `cwd` or any tool-derived real-repository target. Caller-supplied argv
and provider behavior remain under caller trust. Before and after the synchronous child run it records
path kind and SHA-256 bytes or link target for the sorted set returned by
`git ls-files -z --full-name --cached --others --exclude-standard --`. AI Tooling parses raw NUL-
delimited path bytes and sorts the validated repository-relative paths itself; newline, tab,
backslash, and Git quote settings cannot change record boundaries.

The census uses one frozen Git provider. Before repository discovery, AI Tooling resolves bare `git`
once through the caller's `PATH` by direct lookup, realpaths it, requires an existing directly
launchable regular file, and records its identity. Windows command wrappers and drive-relative, UNC,
device, or URL results are invalid. Caller `PATH` selection is explicit operational trust; pack content,
AI configuration, and later environment changes cannot select Git. AI Tooling runs only that absolute
file with `shell: false`, revalidates its identity before repository discovery and each pre/post-census
invocation, and applies the repository gateway if the resolved provider is inside the discovered root.
Before census, that frozen provider runs `git --version`, parses the leading `major.minor.patch` tuple,
and requires Git 2.36.0 or later; an older or unparseable version prevents spawn. The fixed read-only Git provider passes explicit
validated `--git-dir` and `--work-tree` anchors and builds its environment from an empty map with only
required OS values, `GIT_CONFIG_NOSYSTEM=1`, `GIT_CONFIG_GLOBAL` pointing to a tool-created empty
file, and `GIT_OPTIONAL_LOCKS=0`; every other `GIT_*` variable is absent. It also passes
`-c core.excludesFile=<tool-empty-file>`, `-c core.fsmonitor=false`, and
`-c core.untrackedCache=false`. Therefore ambient or repository config cannot redirect the
worktree/index, execute an fsmonitor command, add an external excludes file, or reuse cached untracked
discovery, while repository `.gitignore` and `.git/info/exclude`
remain intentional ignore sources. `.git/**`, ignored paths,
and the installed dependency tree are outside that Git-visible census. Every listed path must
enumerate, read, and hash successfully. Comparison uses the union of both path sets; addition,
removal, kind change, link-target change, or byte change is a checkout change. A pre-census failure
prevents spawn. A post-census failure forbids a success or unchanged claim and requires inspection. An
observed changed censused path is a security failure. This is an observed-unchanged postcondition, not
operating-system write confinement.

The fixed Stage 1 timeout covers 300 seconds from spawn until the whole provider process tree is
quiescent. Process-tree control is established before provider code can execute; inability to do so
emits `process-tree-control-unavailable` and starts no provider code. Each provider runs in a dedicated
Windows Job Object or POSIX process group. After every
root-child outcome, AI Tooling waits until no job or group member remains and drains both pipes to EOF
before taking the post-run census. At the deadline it terminates the entire job or group and performs
the same quiescence and EOF checks; a trusted provider must not detach from that group. If complete
termination cannot be verified, the command emits `EVK_SECURITY_FORMATTER_EXECUTION_FAILED` with
reason `termination-unverified`, performs no success or unchanged claim, and requires operator
inspection. Child stdout and stderr are continuously drained through separate 64 KiB ring buffers so
output volume cannot grow memory without bound; raw child bytes never appear in JSON or default human
diagnostics. Diagnostics
expose only byte counts and truncation flags, and any internal error text redacts the repository root,
disposable root, caller path-like token, every direct-lookup candidate and `PATH` directory, resolved
executable or entry, and installed package root. Raw operating-system and module-resolution errors are
mapped to constructed stable messages rather than surfaced. The same mechanism can
exercise Biome, Prettier, dprint, or another explicitly selected formatter without making its
configuration tool-owned. Stage 1 requires this gate for `devkit`; automatic formatter discovery or
configuration remains outside Stage 1.
...
- **Formatter-provider resolver:** freezes and validates a caller-explicit direct executable or
  repository-local Node entry without acquisition, inference, or shell fallback.
- **Formatter-audit runner:** invokes the frozen provider with exact argv, fixed environment,
  disposable `cwd`, bounded output, timeout, and pre/post checkout census.
...
- the affected repository-relative path, resource identifier, or redacted external-provider
  identifier;
- whether an unsafe write was observed and whether AI Tooling itself opened any repository path for
  write;
...
Formatter-provider diagnostics use `EVK_SECURITY_FORMATTER_PROVIDER_INVALID` with reason
`provider-token-missing`, `unsupported-direct-wrapper`, `invalid-direct-location`, `direct-entry-not-file`,
`direct-entry-not-launchable`, `direct-containment-failed`, `invalid-node-specifier`,
`root-manifest-invalid`, `root-manifest-containment-failed`, `undeclared-node-package`,
`node-modules-anchor-invalid`, `installed-package-outside-root`,
`parent-or-global-resolution`, `cross-package-resolution`, `resolved-entry-not-file`,
`provider-identity-changed`, `provider-trust-changed`, or `installed-tree-identity-changed`.
`EVK_SECURITY_FORMATTER_PROVIDER_UNAVAILABLE` uses reason `direct-executable-missing`,
`root-manifest-missing`, `repository-local-package-missing`, or `node-entry-resolution-failed`.
`EVK_SECURITY_FORMATTER_EXECUTION_FAILED` uses reason `environment-unavailable`,
`process-tree-control-unavailable`, `spawn-error`, `terminated-by-signal`, `timeout`, or
`termination-unverified`. `EVK_OUTPUT_FORMATTER_REJECTED` uses reason `exit-nonzero`, and
`EVK_OUTPUT_FORMATTER_CONFLICT` uses reason `registered-bytes-changed` for changed registered bytes in
the disposable copy. `EVK_SECURITY_FORMATTER_CHECKOUT_CHANGED` uses reason
`checkout-census-changed` for a changed path in the defined Git-visible real-checkout census.
`EVK_SECURITY_FORMATTER_CHECKOUT_UNVERIFIED` uses `git-provider-unavailable`,
`git-provider-invalid`, `git-provider-identity-changed-before-spawn`, `git-version-unsupported`, or
`pre-census-failed` when spawn was prevented. It uses `git-provider-identity-changed-after-spawn` or
`post-census-failed` when a quiescent child tree exists but the trusted Git provider or final
enumeration, read, or hash could not be verified.

Every machine outcome contains every field. `providerMode` is exactly `"direct"` or `"node-entry"`.
`requestedProvider` is a string or `null`: it is `null` only when the CLI form omitted its provider
token, preserves a supplied bare direct token or Node module specifier, and represents a path-like
direct token as exactly `<redacted>/<basename>`, with only its final basename preserved. `reason` is `null` on success
or one stable reason above. `formatterExitStatus` is an integer or `null`; `formatterSignal` is a
string or `null`; `affectedPaths` is a sorted unique array of `/`-separated repository-relative paths;
`stdoutBytes` and `stderrBytes` are nonnegative safe integers; and `stdoutTruncated` and
`stderrTruncated` are booleans. Before a child starts, the status and signal are `null`, paths are `[]`,
counts are `0`, and truncation flags are `false`. Resolved absolute paths and raw child output are
never emitted. `affectedPaths` contains only paths for the primary outcome: changed real-census paths
for checkout change, changed registered disposable paths for formatter conflict, and `[]` otherwise.
Provider syntax or trust failure exits `2`; an unavailable provider, environment or process-tree-
control failure, pre-spawn Git-provider/version/census failure, spawn error, signal, timeout, or
unverified termination exits `4`;
a nonzero formatter exit completed before the deadline or registered-byte conflict exits `1`; and a
post-spawn Git-provider identity or census failure, or observed real-checkout change exits `3`.
Unverified termination takes precedence because no stable quiescent state exists; then post-spawn
Git-provider identity or census failure takes precedence because comparison is incomplete. Otherwise the order is
checkout change, registered-byte conflict, timeout, nonzero exit, signal, then spawn failure. Secondary child
status remains in the exact fields without changing the primary code. No outcome triggers acquisition
or an implicit shell fallback.

A missing formatter token maps to `provider-token-missing`. A missing root manifest maps to
`root-manifest-missing`; invalid JSON or invalid dependency-map shape
maps to `root-manifest-invalid`; and a gateway refusal maps to `root-manifest-containment-failed`.
A missing `node_modules` anchor maps to `repository-local-package-missing`. An existing anchor that is
a link, reparse point, non-directory, or outside the repository maps to `node-modules-anchor-invalid`.
A pre-spawn root-manifest digest or dependency-declaration change maps to `provider-trust-changed`;
provider-file identity change maps to `provider-identity-changed`; installed anchor, package-root, or
containment change maps to `installed-tree-identity-changed`.
Failure to construct the exact temporary directories, trusted Windows-directory values, or fixed
environment maps to `environment-unavailable`; failure to establish tree control before execution
maps to `process-tree-control-unavailable`.

The pre-spawn Git-provider, version, and census reasons confirm no formatter process started and tell
the operator to repair or upgrade the trusted Git installation or path-read access. The post-spawn Git-
identity/census reasons and `termination-unverified` never claim the
checkout unchanged; they tell the operator to stop related processes, inspect repository state, and
restore any damaged path from reviewed VCS bytes or a verified backup before retrying.
...
- `0`: success;
- `1`: validation failure, drift, or conflict confined to the disposable formatter copy;
- `2`: invalid command or configuration;
- `3`: ownership conflict, observed real-checkout change, or required user action;
...
4. Every repository content input and every repository-local path the tool reads, creates, renames,
    replaces, or deletes is path-contained. This includes config, overrides, repository-local packs,
    current and candidate outputs, `ai-tooling.lock.json`, read-only `.gitignore` and formatter-config
    checks, the repository-root package manifest used for formatter-provider validation, reports, and
    all `.ai-tooling/**` state, journals, backups, previews, stale locks, and temporary paths. Symbolic
    links, Windows junctions and other reparse points, UNC escapes, portable-key collisions, and
    changed ancestors fail closed. Ordinary Node module loading for the already trusted engine and
    the caller-explicit installed formatter dependency tree are outside this repository-content
    containment domain under their separate trust contracts; source-specific trust validation
    governs later pack acquisition.
...
Trusted runtime module paths, caller-explicit direct providers outside the repository, installed Node-
entry files and dependency modules, fixed Git-provider internals, and fixed-argv operating-system
provider internals are outside this repository-content gateway and retain their separate trust
contracts. A direct provider inside the repository passes the gateway. Formatter Node entries must
still satisfy the explicit root-manifest, installed-tree, package-root, and entry-resolution contract
above. Any repository content path returned by a provider is validated by the gateway before AI
Tooling uses it.
...
the self-hosting gate runs `doctor --formatter-check --node-entry @biomejs/biome/bin/biome -- check
--write .`. The explicitly selected entry is resolved from `devkit`'s installed direct dependency,
then AI Tooling launches `process.execPath` plus the absolute entry with `shell: false`, the fixed
minimal environment, bounded output, the 300-second timeout, and the disposable repository as `cwd`.
The command proves that no repository-lock or registered-output byte changed, the semantic
configuration digest is unchanged, AI Tooling opened no real-checkout path for write, and the defined
Git-visible pre/post census is identical. It does not claim OS-level confinement of the caller-trusted
formatter. This contains the existing hook deviation without adopting it as product policy.
...
path containment, digest checks, formatter-provider parsing, frozen resolution, environment creation,
outcome precedence and redaction, untrusted run-lock parsing, and error serialization.
...
formatter-provider-token-missing
formatter-direct-wrapper-rejected-on-windows
formatter-direct-native-explicit
formatter-node-entry-explicit
formatter-node-entry-invalid
formatter-node-entry-unavailable
formatter-node-entry-disposable-working-directory
formatter-node-entry-outside-trusted-graph
formatter-node-entry-process-execpath-exact-argv
formatter-provider-fixed-environment
formatter-provider-identity-swap
formatter-provider-trust-swap
formatter-provider-no-acquisition-or-inference
formatter-provider-nonzero-signal-timeout
formatter-provider-process-tree-quiescence
formatter-provider-output-redaction
formatter-real-checkout-census
formatter-root-manifest-invalid
formatter-git-provider-substituted
formatter-git-provider-identity-swap
formatter-census-git-version-unsupported
formatter-checkout-census-unverified
...
compatibility. Formatter cases additionally cover a decoy disposable `node_modules`, parent/global
and `NODE_PATH` fallbacks, a positive pnpm junction on Windows, built-in and `#imports` specifiers,
drive-relative/UNC/URL/percent/dot/backslash input, absent/non-file/cross-package entries, exact argv
with metacharacters, `.cmd`/`.bat`/`.ps1` wrappers, `BIOME_BINARY` and `NODE_OPTIONS` poisoning,
nonzero/signal/timeout after partial disposable writes, bounded raw-output suppression, and a real-
checkout census mutation. A lingering same-group descendant and injected unverified Job Object or
process-group termination prove that every outcome waits for process-tree quiescence and pipe EOF
before census. Injected pre- and post-census Git, enumeration, read, and hash failures prove that no
provider starts without a baseline and no unchanged claim survives an incomplete final comparison.
The census fixture injects hostile caller `GIT_*` redirects and config counts, repository
`core.excludesFile`, executable `core.fsmonitor`, enabled `core.untrackedCache`, and newline/tab
filenames. It substitutes a fake caller-PATH Git and swaps the frozen Git identity before pre- and
post-census calls; selection is explicit, later swaps fail at the defined phase, no fsmonitor process
starts, no cache supplies untracked state, and NUL parsing preserves paths.
AI Tooling itself performs no partial managed mutation, command construction from
metadata, acquisition, implicit shell fallback, or user-content loss. The malicious trusted-child
fixture is different: checkout-change detection occurs after possible damage, does not roll it back,
and reports stop-and-restore-from-VCS-or-backup recovery action instead of a preservation claim.
...
shadowing, golden-output, and deterministic-render fixtures. The direct-provider and explicit
Node-entry formatter fixtures prove frozen absolute resolution, `process.execPath` for the Node
entry, exact argv, `shell: false`, the fixed environment, disposable `cwd`, bounded output, timeout,
verified process-tree quiescence, stable outcome precedence including unverified termination, no
acquisition or inference, fail-closed pre/post census errors, and an unchanged defined real-checkout
census. The exact Biome entry passes on all three operating systems, including a repository-local
pnpm junction on Windows. Phase 4
...
16. No executable or activatable pack asset is copied, installed, enabled, or executed without the
    required source trust and capability consent. Formatter execution requires the separate caller-
    explicit direct or Node-entry provider selection under invariant 29.
...
29. Human-owned consumer formatter configuration excludes the repository lock itself and every output
    ...
    AI Tooling launches direct providers and explicitly selected repository-local Node entries with
    `shell: false`, exact argv, fixed environment, bounded output, timeout, and disposable `cwd`; it
    never inserts or infers a shell. Invalid or unavailable providers trigger no acquisition or
    fallback. AI Tooling opens no real-checkout path for write and proves the defined Git-visible
    census unchanged after the caller-trusted, non-sandboxed child; shared presets remain neutral.
...
32. One containment gateway protects every repository-content input, including the root package
    manifest used for formatter trust validation, every repository output, lock, read-only repository-
    config check, and `.ai-tooling/**` path, and fails closed when any link, reparse point, or validated
